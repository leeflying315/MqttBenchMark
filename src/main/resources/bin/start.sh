...
#java ${JAVA_OPTS} -Dlog4j.configurationFile=${base_dir}/conf/log4j2-production.xml -Dbase.dir=${base_dir} -jar ${name} >> 1.log  2>&1 &
nohup java ${JAVA_OPTS} -Dlog4j.configurationFile=${base_dir}/conf/log4j2.xml -Dbase.dir=${base_dir} -jar ${name} >> /dev/null 2>&1 &
