...
#java ${JAVA_OPTS} -Dlog4j.configurationFile=${base_dir}/conf/log4j2.xml -Dbase.dir=${base_dir} -cp ./${name}:${base_dir}/conf ${main_class} >> 1.log  2>&1 &
nohup java ${JAVA_OPTS} -Dlog4j.configurationFile=${base_dir}/conf/log4j2.xml -Dbase.dir=${base_dir} -cp ./${name}:${base_dir}/conf ${main_class} >> /dev/null 2>&1 &
